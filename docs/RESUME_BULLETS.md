...
*   **LLM Routing Resilience (Circuit Breaker v2):** Migrated semantic router from Gemini (Rate-Limited 429) to `openrouter/qwen/qwen3.6-plus:free` — infinite free RPD, Gemini quota fully preserved for end-user sessions. Expanded intelligent circuit breaker to a 7-model fallback cascade including Qwen3 Coder 480B MoE (OpenRouter + SiliconFlow) and instantaneous failover from rate-limited 405B instances (Hermes 3) to agile 70B models (Llama 3.3). Achieved true hardware-fault tolerance and deep provider independence without compromising output quality.
