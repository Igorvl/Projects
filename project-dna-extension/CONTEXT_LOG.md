...
## ✅ T-09 STATUS: CLOSED (v2.0.36 — 2026-03-21)
**AI Studio text formatting and fragmentation bug fixed.**
- **Bug:** Text generated by AI Studio was saved with missing parts (e.g., "Ко", "льно") and improper newlines between words.
- **Root Cause:** AI Studio gRPC streams text as incremental tokens (not cumulative). Our scanner was dropping tokens that lacked spaces via `includes(' ')`, and our assembler was incorrectly separating incremental tokens using `\n` instead of `""`.
- **Solution:** 
  - Refactored `2B-bis` (JSON success) and `2D` (raw regex) paths to use **precise structural alignment**: extracting `[null, "token"]` to cleanly catch all text components, even without spaces.
  - Replaced `join('\n')` with `join('')` so that sub-word tokens form perfectly contiguous strings.
