...
# ─────────────────────────────────────────────────────────
# AGENTIC INTERCEPTOR — уникальный маркер в теле picker-сообщения.
# HTML-комментарий: OW-UI не рендерит его, но хранит в raw content.
# Проверка по нему не зависит от markdown-форматирования (**bold** etc.)
# ─────────────────────────────────────────────────────────
DNA_PICKER_TAG = "<!-- DNA_PICKER -->"
...
async def tts_chatterbox(text: str, voice: str = "chatterbox", max_retries: int = 3) -> bytes | None:
    """Generate speech using Chatterbox TTS (premium English)"""
    global CHATTERBOX_AVAILABLE
...
async def process_book(job_id: str, text: str, voice: str = "af_heart"):
    ...
    try:
        ...
        is_heavy_tts = is_chatterbox_voice(voice)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if audio:
                ...
                if consecutive_failures >= 5:
                    logger.error(f"Book [{job_id[:8]}]: {consecutive_failures} consecutive failures! Aborting.")
                    break
                elif consecutive_failures >= 3:
                    logger.warning(f"Book [{job_id[:8]}]: waiting 120s for TTS recovery...")
...
async def chat_completions(request: ChatRequest):
    ...
    if project_slug == "open-webui" or not project_slug:
        ...
        # ─────────────────────────────────────────────────────────────────
        # ⚡ FAST PATH: Проверяем ДО вызова semantic_auto_detect.
        #
        # Если последнее сообщение — цифра И предпоследнее от ассистента
        # содержит наш DNA_PICKER_TAG (или fallback "Система DNA") —
        # значит пользователь отвечает на picker. Пропускаем дорогой
        # LLM-вызов и сразу идём в БЛОК 1 с detected_slug = "UNKNOWN".
        #
        # Это экономит 20-30 секунд "думания" GLM на каждый ответ "5".
        # ─────────────────────────────────────────────────────────────────
        last_content  = str(raw_messages[-1].get("content", "")).strip()
        prev_content  = str(raw_messages[-2].get("content", "")) if len(raw_messages) >= 2 else ""

        is_picker_reply = (
            last_content.isdigit() and
            (
                DNA_PICKER_TAG in prev_content or          # надёжный маркер
                "Система DNA" in prev_content              # fallback для старых сообщений
            )
        )

        if is_picker_reply:
            # Пропускаем semantic_auto_detect, сразу в БЛОК 1
            detected_slug = "UNKNOWN"
            logger.info("[INTERCEPTOR] Fast path: detected numeric response to picker")
        else:
            detected_slug = await semantic_auto_detect(raw_messages, db, call_with_key_rotation)

        # ─────────────────────────────────────────────────────────────────
        # БЛОК 1: Пользователь отвечает цифрой на picker.
        # Парсим выбор, кэшируем, вырезаем технические сообщения.
        # ─────────────────────────────────────────────────────────────────
        if detected_slug == "UNKNOWN" and len(raw_messages) >= 3:
            prev_msg_content = str(raw_messages[-2].get("content", ""))

            # Проверка по HTML-маркеру (надёжно) ИЛИ по тексту (fallback).
            # Не зависит от того, сохранил ли OW-UI markdown **bold** или нет.
            picker_detected = (
                DNA_PICKER_TAG in prev_msg_content or
                "Система DNA" in prev_msg_content
            )
            ...
            if picker_detected:
                ...
                try:
                    ...
                    if 1 <= choice <= len(projects):
                        ...
                        # Кэшируем выбор по хэшу ПЕРВОГО сообщения диалога
                        ...
                        # 🔥 ВЫРЕЗАЕМ ВСЮ picker-историю, а не только последние 2!
                        # Без этого GLM видит накопленный паттерн picker→"5"→picker
                        # и воспроизводит его снова на каждый запрос.
                        first_picker_idx = None
                        for idx_p, msg_p in enumerate(raw_messages):
                            c_p = str(msg_p.get("content", ""))
                            if (msg_p.get("role") == "assistant" and
                                    (DNA_PICKER_TAG in c_p or "Система DNA" in c_p)):
                                first_picker_idx = idx_p
                                break

                        if first_picker_idx is not None:
                            raw_messages = raw_messages[:first_picker_idx]
                            logger.info(f"[INTERCEPTOR] ✅ Вся picker-история вырезана (с msg[{first_picker_idx}]). Проект: {detected_slug}")
                        else:
                            raw_messages = raw_messages[:-2]
                            logger.info(f"[INTERCEPTOR] ✅ Мусор стерт (fallback -2). Проект: {detected_slug}")

                    elif choice == len(projects) + 1:
                        # Пользователь выбрал "Мусор"
                        detected_slug = "trash"
                        # Та же чистка: убираем всю picker-историю
                        first_picker_idx = None
                        for idx_p, msg_p in enumerate(raw_messages):
                            c_p = str(msg_p.get("content", ""))
                            if (msg_p.get("role") == "assistant" and
                                ...
                                first_picker_idx = idx_p
                                break
                        raw_messages = raw_messages[:first_picker_idx] if first_picker_idx is not None else raw_messages[:-2]
                        logger.info("[INTERCEPTOR] 🗑️ Пользователь выбрал: Мусор")

                    else:
                        logger.warning(f"[INTERCEPTOR] Цифра вне диапазона: {choice} (проектов: {len(projects)})")

                except ValueError:
                    logger.info(f"[INTERCEPTOR] Не цифра: '{user_reply}' — игнорируем")

        # ─────────────────────────────────────────────────────────────────
        # БЛОК 2: Роутер не определил проект → показываем picker.
        # ─────────────────────────────────────────────────────────────────
        if detected_slug == "UNKNOWN":
            ...
            async def agentic_interceptor_stream():
                options = "\n".join(
                    [f"{i+1}. {p['slug']} ({p['name']})" for i, p in enumerate(projects)]
                )
                options += f"\n{len(projects)+1}. Мусор (Не сохранять)"

                # DNA_PICKER_TAG — HTML-комментарий. Не отображается пользователю,
                # но остаётся в raw content сообщения и надёжно детектируется в БЛОК 1.
                msg = (
                    f"{DNA_PICKER_TAG}"
                    f"🚦 **Система DNA:** Я не уверен, к какому проекту относится этот чат. "
                    f"Напиши цифру:\n\n{options}"
                )
        ...
        # Успешно определили проект (LLM, кэш или выбор пользователя)
