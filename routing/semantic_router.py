...
# Кэш удачных маршрутов: хранит привязку чатов (по хэшу первого сообщения) к проектам
# Формат: {'e99a18c4...': 'ksar-me'}
ROUTING_CACHE = {}

# Кэш провальных маршрутов с TTL.
# Если LLM уже фейлил для того же текста — не повторять запрос ещё 5 мин.
# Анти -спам для "Test failed: ..."
# Формат: {'e99a18c4...': timestamp}
ROUTING_FAILED_CACHE: dict[str, float] = {}
ROUTING_FAILED_TTL = 300  # 5 минут не повторять
...
async def semantic_auto_detect(messages: list, db_instance, call_llm_func=None) -> str:
    ...
    # 2b. Кэш провалов: если этот же текст уже фейлил недавно — не повторяем,
    # чтобы не спамить OpenRouter одними и теми же запросами (анти-спам)
    if msg_hash in ROUTING_FAILED_CACHE:
        elapsed = time.time() - ROUTING_FAILED_CACHE[msg_hash]
        if elapsed < ROUTING_FAILED_TTL:
            remaining = int(ROUTING_FAILED_TTL - elapsed)
            print(f"[SEMANTIC ROUTER] ⏱  Кэш провалов: '{user_text[:30]}' — пропускаю ({remaining}s TTL осталось)")
            return "UNKNOWN"
        else:
            # TTL истёк — даём ещё один шанс
            del ROUTING_FAILED_CACHE[msg_hash]
    ...
    try:
        ...
        if call_llm_func:
            res = await call_llm_func(
                mid="openrouter/openai/gpt-oss-120b:free",
        ...
        if detected_slug in valid_slugs_lower:
            ...
            # Убираем из failed-кэша, если был там
            ROUTING_FAILED_CACHE.pop(msg_hash, None)
            print(f"✅ [SEMANTIC ROUTER] Авто-маршрутизация: [{actual_slug}]")
            return actual_slug

    except Exception as e:
        print(f"❌ [SEMANTIC ERROR] Ошибка авто-маршрутизации: {e}")
        # Кэшируем фейл: не будем повторять {ROUTING_FAILED_TTL}s
        ROUTING_FAILED_CACHE[msg_hash] = time.time()
        print(f"[SEMANTIC ROUTER] Провал закэширован: '{user_text[:30]}' ({ROUTING_FAILED_TTL}s TTL)")
